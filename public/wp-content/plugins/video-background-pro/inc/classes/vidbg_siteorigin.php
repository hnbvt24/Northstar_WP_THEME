 ...
 * @copyright Copyright (c) Push Labs (blake@pushlabs.co)
 * @since 3.0.0
 */
	...
	class Vidbg_SiteOrigin {
		...
		public function generate_shortcode_before_row( $output, $grid_item, $grid_attributes ) {
			...
			if ( function_exists( 'vidbgpro_install_plugin' ) ) {
				...
				if ( 'youtube' === $this->vidbg_atts['type'] && empty( $this->vidbg_atts['youtube_url'] ) ) {
					// var_dump( 'no youtube video exists' );
					return;
				}

				// If type is Vimeo and Vimeo URL param is empty, quit.
				if ( 'vimeo' === $this->vidbg_atts['type'] && empty( $this->vidbg_atts['vimeo_url'] ) ) {
					return;
				}

				// If type is self-host and mp4 amd webm param are both empty, quit.
				if ( 'self-host' === $this->vidbg_atts['type'] && empty( $this->vidbg_atts['mp4'] ) && empty( $this->vidbg_atts['webm'] ) ) {
			...
			if ( array_key_exists( 'end_frame_poster', $this->vidbg_atts ) ) {
				$this->vidbg_atts['end_frame_poster'] = $this->vidbg_atts['end_frame_poster'] === true ? 'true' : 'false';
			}

			if ( array_key_exists( 'frontend_play_button', $this->vidbg_atts ) ) {
				$this->vidbg_atts['frontend_play_button'] = $this->vidbg_atts['frontend_play_button'] === true ? 'true' : 'false';
			}

			if ( array_key_exists( 'frontend_volume_button', $this->vidbg_atts ) ) {
				$this->vidbg_atts['frontend_volume_button'] = $this->vidbg_atts['frontend_volume_button'] === true ? 'true' : 'false';
			}
			...
			// Our getSiteOriginContainer method to add the container class to our SiteOrigin row.
			$add_container_to_row = "
      VideoBackgroundPro.createSiteOriginContainer(
        '" . $unique_class . "',
        '" . $container_class . "'
      );
      ";
