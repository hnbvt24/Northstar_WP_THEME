	...
	class Vidbg_WPBakery {
		...
		public function generate_shortcode() {
			...
			if (function_exists('vidbgpro_install_plugin')) {
                ...
                /**
                 * If type is "self-host" and mp4 and webm params are both empty, quit.
                 *
                 * We must check that type is not set because WPBakery will not add type to
                 * the vidbg_atts if it is set to the default, which in this case is "self-host".
                 *
                 * Issue was found in VidbgPro 4.0 and fixed in VidbgPro 4.0.1
                 */
                ...
                if (isset($this->vidbg_atts['type']) && 'youtube' === $this->vidbg_atts['type'] && ! isset($this->vidbg_atts['youtube_url'])) {
                    return;
                }

                // If type is "vimeo" and Vimeo URL param is empty, quit
                if (isset($this->vidbg_atts['type']) && 'vimeo' === $this->vidbg_atts['type'] && ! isset($this->vidbg_atts['vimeo_url'])) {
                    return;
                }
            } else {
			...
			// Our code to add the container class to the container so we can target the VC Row
