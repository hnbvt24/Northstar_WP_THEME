...
define( 'VIDBGPRO_PLUGIN_VERSION', '4.0.3' );
 ...
 * @since 2.2.0
 *
 * @param Boolean $is_enabled the decision
 * @return Boolean
 */
 ...
 * @since 4.0.3
 *
 * @param Boolean $do_not_track The option.
 * @return Boolean
 */
...
function vidbgpro_load_vc_integration() {
	...
	// Don't use the Visual Composer integration if the filter returns false
	if ( vidbgpro_enable_vc() === false ) {
		return;
	}

	// Determine which VC integration class to use
	if ( vidbgpro_integration_use_old_data_model() === true ) {
		$class = 'vidbg_vc_integration';
	} else {
		$class = 'vidbg_wpbakery';
	}

	if ( class_exists( 'Vc_Manager' ) ) {
		require_once VIDBGPRO_PLUGIN_PATH . 'inc/classes/' . $class . '.php';
	}

	if ( $class = 'vidbg_wpbakery' ) {
...
function vidbgpro_load_siteorigin_integration() {
	...
	// Don't use the SiteOrigin integration if the filter returns false
	...
	// Determine which VC integration class to use
	if ( vidbgpro_integration_use_old_data_model() === true ) {
		$class = 'vidbg_siteorigin_integration';
	} else {
...
function vidbgpro_enqueue_admin_scripts() {
	...
	wp_localize_script(
		'vidbgpro-admin-backend', 'vidbgpro_localized_text', array(
...
function vidbgpro_enqueue_frontend_scripts() {
	...
	wp_enqueue_script( 'vidbgpro-youtube', 'https://www.youtube.com/iframe_api' );
...
function vidbgpro_init_footer() {
	if ( is_page() || is_single() || is_home() && get_option( 'show_on_front' ) == 'page' ) {
		...
		$shortcode_atts = array(
			...
			'end_frame_poster'       => ( $end_frame_poster_meta == 'on' ) ? 'true' : 'false',
			...
			'frontend_play_button'   => ( $frontend_play_button_meta == 'on' ) ? 'true' : 'false',
			'frontend_volume_button' => ( $frontend_mute_button_meta == 'on' ) ? 'true' : 'false',
			'frontend_position'      => ( $frontend_position_meta == 'bottom-right' ) ? 'bottom-right' : $frontend_position_meta,
			'source'                 => 'Metabox',
		);
